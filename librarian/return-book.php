                ...
                <div class="content-header">
                    ...
                    <div class="leftside-content-header">
                        <ul class="breadcrumbs">
                            ...
                            <li><a href="javascript:avoit(0)">Return Book</a></li>
                        </ul>
                    </div>
                </div>
                <!-- =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= -->
                        ...
                        <div class="panel-content">
                            <div class="table-responsive">
                                <table id="basic-table" class="table-bordered data-table table table-striped nowrap table-hover" cellspacing="0" width="100%">
                                    ...
                                    <tbody>
                                    	<?php
                                    		$result = mysqli_query($con,"SELECT `issue_books`.`id`,`issue_books`.`book_id`,`issue_books`.`issue_date`, `students`.`fname`,`students`.`lname`,`students`.`roll`,`students`.`phone`,`books`.`book_name`,`books`.`book_image`
FROM `issue_books`
                                    		...
                                    		while($row = mysqli_fetch_assoc($result)){
                                    			?>
													<tr>
                                                        ...
                                                        <td><a href="return-book.php?id=<?= $row['id'];?>&bookid=<?= $row['book_id'];?>"><button class="btn btn-primary">Return Book</button></a></td>
                                    				</tr>
                                    			<?php
                                    		}
                                    	?>
